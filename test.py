...
def extract_feature(model,dataloaders, view_index = 1):
    ...
    count = 0
    block_num = sum([int(i) for i in config['block'].split(",")]) + len([int(i) for i in config['block'].split(",")])
    for data in dataloaders:
        img, label = data
        n, c, h, w = img.size()
        count += n
        print(count)
        block = [int(i) for i in config['block'].split(",")]
        ff = torch.FloatTensor(n,1024,block_num).zero_().cuda()
        for i in range(2):
            if(i==1):
                img = fliplr(img)
        ...
        fnorm = torch.norm(ff, p=2, dim=1, keepdim=True) * np.sqrt(block_num)
